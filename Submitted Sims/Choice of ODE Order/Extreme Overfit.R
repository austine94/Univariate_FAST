...
y <- y + noise
bas <- create.bspline.basis(range(time), nbasis = 150, norder = 12)
bas <- fdPar(bas, 2, 1)
...
for(j in 1:ncol(y)){
  ...
  for(i in 1:5){
    fast_run <- fast(time, xfd, (i+5), yfd$fd[j], threshold)
...
result_df_varying_m_order2 <- result_df[-1,]
rownames(result_df_varying_m_order2) <- c("Model 1", "Model 2", "Model 3", "Model 4",
                                   "Model 5", "Model 6", "Model 7", "Model 8",
                                   "Model 9")
colnames(result_df_varying_m_order2) <- as.character(1:5)
print("Varying m Order 2 Sim: Done")
