...
pdafast_manual_threshold <- function(time, fd, m, threshold, basis = NULL, penalty_order = 1,
  ...
  return(list(test_stat = cusum, detection_time = detection, threshold = threshold))
...
eval_mat_scale <- function(eval_mat){
  ...
  test_stat <- apply(null_mat, 2, cumsum)   #cumsum #will be N(0, n \sigma^2)
  ...
  rescaled_test_stat <- sweep(scaled_test_stat, 1, sd_value_bm, "/") #N(0,1)
...
# false_alarm_control <- function(test_stats, start_time, pfa){
#   
#   pre_anomaly <- test_stats[1:start_time,]
#   max_stats <- apply(pre_anomaly, 2, max)
#   threshold_to_control_pfa <- quantile(max_stats, 1-pfa)
#   post_anomaly <- test_stats[(start_time+1):nrow(test_stats),]
#   
#   detections <- rep(NA, ncol(test_stats))
#   for(i in 1:ncol(post_anomaly)){
#     if(any(post_anomaly[,i] > threshold_to_control_pfa)){
#       detections[i] <- min(which(post_anomaly[,i] > threshold_to_control_pfa))
#     }
#   }
