...
time <- 1:500
covar <- squared_exp_covar(1:length(time), 40, 0.05)
lt <- length(time)

n_train <- 100
n_test <- 500

x_underlying <- exp(-time/lt)
...
for(j in 1:ncol(y)){
  ...
  fast_run <- fast(time, xfd, 1, yfd$fd[j], threshold = threshold)
...
bas <- create.bspline.basis(range(time), nbasis = 100, norder = 6)
yfd <- smooth.basis(time, y, bas)
...
result_df_order1 <- result_df[-1,-3]
rownames(result_df_order1) <- c("Model 1", "Model 2", "Model 3", "Model 4", "Model 5", "Model 6",
                         "Model 7", "Model 8", "Model 9")
print("Order 1 Sim: Done")