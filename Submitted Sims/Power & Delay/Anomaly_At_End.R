...
for(i in 1:ncol(y)){
  ...
  y[401:500, i] <- y[401:500,i] + a[3] + a[1]*((time[401:500] - time[400]) / (lt)) +
    a[2]*exp(-time[401:500]-time[400])
}
bas <- create.bspline.basis(range(time), nbasis = 60, norder = 6)
yfd <- smooth.basis(time, y, bas)
...
covar2 <- squared_exp_covar(401:500, 35, 0.3)  #for shape anomaly
