...
for(j in 1:ncol(y)){
  ...
  fast_run <- fast(time, xfd, 3, yfd$fd[j], threshold = threshold)
...
for(j in 1:ncol(y)){
  ...
  fast_run <- fast(time, xfd, threshold, yfd$fd[j], threshold = threshold)
...
result_df_order3 <- result_df[-1,-3]
rownames(result_df_order3) <- c("Model 1", "Model 2", "Model 3", "Model 4", "Model 5", "Model 6",
                                "Model 7", "Model 8", "Model 9")
print("Order 3 Sim: Done")