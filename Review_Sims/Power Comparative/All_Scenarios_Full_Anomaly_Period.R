...
for(i in 1:ncol(y)){
  ...
  y[1:500, i] <- y[1:500, i] + a[3] + a[1]*((time[1:500]) / (lt)) + a[2]*exp(-time[1:500])
}
bas <- create.bspline.basis(range(time), nbasis = 60, norder = 6)
yfd <- smooth.basis(time, y, bas)
y_eval <- eval.fd(time, yfd$fd)
...
for(i in 1:ncol(y)){
  a <- sample(c(-1,1), 3, TRUE) * runif(3, 0.01, 0.1)
  y[(start_times[i]:500), i] <- y[(start_times[i]:500), i] + a[3] +
                                a[1]*((start_times[i]:500) / (lt)) +
                                a[2]*exp(-(start_times[i]:500))
}
bas <- create.bspline.basis(range(time), nbasis = 60, norder = 6)
yfd <- smooth.basis(time, y, bas)
y_eval <- eval.fd(time, yfd$fd)
