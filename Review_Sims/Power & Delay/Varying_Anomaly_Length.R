...
for(k in 1:length(s_len_vec)){ #iterate over each anomaly region length
  ...
  for(i in 1:ncol(y)){
    a <- runif(4, 0.5, 1.5)
    b <- runif(1, 0, 0.5)
    y[(start_period:end_period), i] <- y[(start_period:end_period),i] + b[1] + a[1]*((time[(start_period:end_period)] - start_period) / (lt)) +
      a[2]*((time[(start_period:end_period)] - start_period)^2/(lt)^2) + a[3]*((time[(start_period:end_period)] - start_period)^3/(lt)^3)
    + a[4]*((time[(start_period:end_period)] - start_period)^4/(lt)^4)
  }
...
################
#Model 9 - GP Residuals, Polynomial Anomaly
#################
