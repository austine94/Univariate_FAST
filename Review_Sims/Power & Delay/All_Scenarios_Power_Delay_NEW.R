...
lt <- length(time)
mss_scale_factor <- 3 #as in Huang (2019)
tvd_scale_factor <- 2.45
...
for(j in 1:ncol(y)){
  ...
  tvd_run <- tvd_anomaly_detection(combined_fd, mss_scale_factor, tvd_scale_factor)
  #we use both metrics to detect anomalies
  tvd_anomalies <- unique(c(tvd_run$mss_anomalies, tvd_run$tvd_anomalies))
  if(length(tvd_anomalies) > 0){ #if at least one anomaly detected
...
new_result_df_row <- c(fast_detected/n_test, tvd_detected / n_test, fast_ADD, 
...
################
#Model 3 - GP Residuals, Polynomial Anomaly
#################
...
for(i in 1:ncol(y)){
  a <- runif(4, 0.5, 1.5)
  b <- runif(1, 0, 0.5)
  y[(start_times[i]:end_times[i]), i] <- y[(start_times[i]:end_times[i]),i] + b[1] +
...
############
#Model 9 - T Residuals, Polynomial Anomaly
############
...
rownames(result_df) <- c("Model 1", "Model 2", "Model 3", "Model 4", "Model 5", "Model 6",
