...
for(i in 1:ncol(y)){
  a <- runif(3, 0.5, 1.5)
  y[101:200, i] <- y[101:200,i] + a[1] + a[2]*(time[101:200] - time[100]/lt) +
                   a[3]*exp(-time[101:200]^2)
}
bas <- create.bspline.basis(range(time), nbasis = 60, norder = 6)
yfd <- smooth.basis(time, y, bas)
y_eval <- eval.fd(time, yfd$fd)
...
for(i in 1:ncol(y)){
  ...
  y[(start_times[i]:end_times[i]), i] <- y[(start_times[i]:end_times[i]),i] + a[3] +
    ...
    a[2]*exp(-(start_times[i]:end_times[i])^2)
}
bas <- create.bspline.basis(range(time), nbasis = 60, norder = 6)
yfd <- smooth.basis(time, y, bas)
y_eval <- eval.fd(time, yfd$fd)
...
############
#Model 9 - T Residuals, Magnitude Anomaly
############
...
result_df_partial_anomaly <- result_df[-1,]
rownames(result_df_partial_anomaly) <- c("Model 1", "Model 2", "Model 3", "Model 4", "Model 5", "Model 6",
