...
threshold <- 1.9 #one test per obs
...
for(j in 1:ncol(y)){
  ...
  for(k in 1:(n_test+1)){
    fast_run <- fast_end_of_day(time, xfd, 2, combined_fd_smooth[k],
                                threshold = threshold)
...
new_result_df_row <- c(fast_detected / (n_test*(n_train+1)),
                       tvd_detected / (n_test*(n_train+1)), 
...
result_df_offline_partial <- result_df[-1,]
rownames(result_df_offline_partial) <- c("Model 1", "Model 2", "Model 3", "Model 4", "Model 5", "Model 6",
